...
4. Vytvorte príkaz `prikazy`, pomocou ktorého sa vypíše zoznam všetkých príkazov, ktoré sa dajú v hre použiť.

    ```python
    > prikazy
    V hre je možné použiť tieto príkazy:
    * koniec - ukonci rozohratu hru
    * o hre - zobrazi informacie o hre
    * prikazy - zobrazi zoznam dostupnych prikazov v hre
    ```
...
5. Ak hráč zadá prázdny reťazec (napr. stlačí kláves `ENTER`), tak neurobíte nič.
...
6. V prípade, že hráč zadá príkaz, ktorý neexistuje, tak hra vypíše na obrazovku text:
...
7. Ak teraz napíšete príkaz `koniec`, vypíše na obrazovkú správu `Taký príkaz nepoznám.` a program skončí. Ošetrite toto správanie. 
